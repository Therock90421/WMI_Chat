class MessagesController < ApplicationController
  ...
  def create
    ...
    Rails.logger.info('######################################################')
    Rails.logger.info(params[:chat_room])
    ...
    respond_to do |format|
      if chat.nil?
        ...
        if @message.save
          ...
          if sender.picture?
            picture_url = sender.picture.url
          else
            picture_url = "user-icon.png"
          end
          if current_user.id==@message.user_id
          ActionCable.server.broadcast "chat_channel" , chat:chat ,body:@message.body ,sender_name:now_sender.name,isyourself:true ,sender_id:@message.user_id, picture: picture_url
          else
          ActionCable.server.broadcast "chat_channel" , chat:chat ,body:@message.body ,sender_name:now_sender.name,isyourself:false ,sender_id:@message.user_id, picture: picture_url
          end
          # Rails.logger.info(@message)
          # Rails.logger.info(chat)
          sync_new @message, scope: chat
          Rails.logger.info('发送一次！！！！！！！！！！！！！！！！！！！！！')
          format.js
  ...
  def sendtorobot
    ...
    robot.update_attributes(online: 1)
